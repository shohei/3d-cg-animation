...
function webMain() 
{
  ...
  //シェーダを初期化する
  initGlsl(gl_L, VS_SOURCE, FS_SOURCE);//シェーダのソースは同じ
...
//  mouseOperation(canvasR, cameraR);//kwgSupport.js
  ...
  var animeStart = function()
  {
    ...
    if(flagStart)
    {
      drawScene(gl_L, canvasL);
      drawScene(gl_R, canvasR);
...
function drawScene(gl, canvas0)
{
  // カラーバッファとデプスバッファをクリアする
  gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
  gl.viewport(0, 0, canvas0.width, canvas0.height);
