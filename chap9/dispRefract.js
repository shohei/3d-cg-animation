...
function webMain() {
  ...
  var animeStart = function()
  {
    ...
    if(flagStart)
    {
      ...
      rigid[selectNo].vEuler.y += angleStep * frameTime;//回転角を更新
...
function drawScene()
{
  ...
  var fDispLoc = gl.getUniformLocation(gl.program, 'u_fDisp');
  gl.uniform1f(fDispLoc, fDisp);
...
{ 
  rigid[selectNo].nSlice = parseInt(form2.nSlice.value);
  display();
}
function onClickStack()
{ 
  rigid[selectNo].nStack = parseInt(form2.nStack.value);
  display();
}*/

function onClickDisp()
{ 
  fDisp = parseFloat(form2.fDisp.value);
  display();
}
