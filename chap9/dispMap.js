...
function initBumpMap() 
{
  ...
  //image.src = '../imageJPEG/webgl0.jpg';
  //image.src = '../imageJPEG/char1.jpg';
 ...
 // 画像の読み込み完了時のイベントハンドラを設定する
  ...
  function setTexture() 
  {
    ...
    var samplerLoc = gl.getUniformLocation(gl.program, 'u_samplerBump');
...
{  
  ...
  if(objectNo == 0) rigid.kind = "GRID_PLATE";
  else if(objectNo == 1)rigid.kind = "SPHERE";  
  else if(objectNo == 2) rigid.kind = "TORUS";
  else if(objectNo == 3) rigid.kind = "SUPER";
   
  if(rigid.kind == "TORUS") flagTorus = true;
  var objectLoc = gl.getUniformLocation(gl.program, 'u_objectNo');
  gl.uniform1i(objectLoc, objectNo);
...
function onChangeObjNo()
{
  ...
  if(objectNo == 0) form2.objectName.value = "平面";
  else if(objectNo == 1) form2.objectName.value = "球";
  else if(objectNo == 2) form2.objectName.value = "トーラス";
  else if(objectNo == 3) form2.objectName.value = "超2次曲面";
