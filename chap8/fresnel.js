...
var texCube;
var nRatio = 1.0;//比屈折率
var dispersion = 0.05;//赤に対する緑の分散の増加割合（nGreen/nRed-1)
...
function init()
{
  ...
  rigid[0].diffuse = [0.2, 0.2, 0.8, 1.0];
  rigid[0].ambient = [0.1, 0.1, 0.4, 1.0];
  rigid[0].specular = [0.5, 0.5, 0.5, 1.0];
  rigid[0].shininess = 100.0; 
  rigid[0].vPos = new Vector3(1.0, -1.0, 1.0);
  rigid[0].vEuler = new Vector3(0.0, 0.0, 0.0);
  rigid[0].vSize = new Vector3(1.0, 1.0, 1.0);
  rigid[0].radiusRatio = 0.2;

  rigid[1].kind = "SPHERE";;
  //rigid[1].kind = "CUBE";
  rigid[1].diffuse = [0.8, 0.8, 0.8, 1.0];
  rigid[1].ambient = [0.4, 0.4, 0.4, 1.0];
  rigid[1].specular = [0.5, 0.5, 0.5, 1.0];
  rigid[1].shininess = 100.0;  
  rigid[1].vPos = new Vector3(1.0, 0.0, 1.0);
  ...
  rigid[2].vPos = new Vector3(1.0, 1.0, 1.0);
  rigid[2].vEuler = new Vector3(0.0, 90.0, 90.0);
  rigid[2].vSize = new Vector3(1.0, 1.0, 1.0);
 
  table.vPos = new Vector3(-5.0, 0.0, 0.0);
  table.vEuler = new Vector3(0.0, 0.0, 0.0);
  table.vSize = new Vector3(2.0, 2.0, 2.0);
  table.parts.diffuse = [0.8, 0.8, 0.8, 1.0];
  table.parts.ambient = [0.4, 0.4, 0.4, 1.0];

  chair = new Chair();
  chair.vPos = new Vector3(-4.0, 0.0, 0.0);
  chair.vEuler = new Vector3(0.0, 0.0, 0.0);
  chair.vSize = new Vector3(2.0, 2.0, 2.0);
  chair.parts.diffuse = [0.8, 0.8, 0.8, 1.0];
  chair.parts.ambient = [0.4, 0.4, 0.4, 1.0];
...
function makeTexture()
{
  ...
  for(var i = 0; i < 6; i++)
  {
    ...
    for(var k = 0; k < numObject; k++) 
    {
      ...
      var n = rigid[k].initVertexBuffers(gl);
      rigid[k].draw(gl, n);
    }
    table.draw(gl);
    chair.draw(gl);
...
function drawScene()
{
  ...
  //赤に対する緑の屈折率増加の割合
  var dispLoc = gl.getUniformLocation(gl.program, 'u_dispersion');
  gl.uniform1f(dispLoc, dispersion);
  //透明度
  ...
  drawShadow(cubemapLoc);
  
  gl.bindTexture(gl.TEXTURE_CUBE_MAP, null);
}

function drawShadow(cubemapLoc)
{
  gl.depthMask(false);
  ...
  {
    if(i == selectNo) gl.uniform1i(cubemapLoc, true);
    else gl.uniform1i(cubemapLoc, false);
  ...
  table.shadow = shadow_value;
  table.draw(gl);
  table.shadow = 0;
  chair.shadow = shadow_value;
  chair.draw(gl);
  chair.shadow = 0;
...
{
  var i, flag;
  if(form2.wireframe.checked) flag = false;// for(i = 0; i < numObject; i++){ rigid[i].flagSolid = false; flagSolid = false; }
  else                        flag = true;//for(i = 0; i < numObject; i++){ rigid[i].flagSolid = true; flagSolid = true; }
  for(i = 0; i < numObject; i++) rigid[i].flagSolid = flag;
  table.flagSolid = flag;
  chair.flagSolid = flag;
  display(); 
}
...
function onClickDispersion()
{
  dispersion = parseFloat(form2.dispersion.value);
  display();
}
