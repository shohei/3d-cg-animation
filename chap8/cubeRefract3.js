...
var dog = new Dog();
var plane = [0.0, 0.0, 1.0, 1.0];//床平面(z = -2)
...
{
  //robotの初期設定
  dog.vPos = new Vector3(1.0, -1.5, -1.0);
...
//  rigid.ambient = [0.4, 0.4, 0.3, 1.0];
  ...
  rigid.vPos = new Vector3(-2.0, 0.0, 0.0);
...
function makeTexture()
{
  ...
  for(var i = 0; i < 6; i++)
  {
    ...
    vpMatrix.lookAt(dog.vPos.x, dog.vPos.y, dog.vPos.z + dog.height, 
          dog.vPos.x + target[i].cnt[0], dog.vPos.y + target[i].cnt[1], dog.vPos.z + dog.height + target[i].cnt[2], target[i].up[0], target[i].up[1],target[i].up[2]);
