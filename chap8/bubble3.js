...
function init()
{
  ...
  rigid[1].specular = [0.9, 0.9, 0.9, 1.0];
...
//  rigid[1].flagCubemap = false;
  ...
  form2.filmSpace.value = filmSpace;
...
{  
  ...
  //経過時間
  var timeLoc = gl.getUniformLocation(gl.program, 'u_time');
  gl.uniform1f(timeLoc, elapseTime2);
  //膜の厚さ
  var spaceLoc = gl.getUniformLocation(gl.program, 'u_filmSpace');
  gl.uniform1f(spaceLoc, filmSpace);
...
//  gl.clearColor(0.2, 0.8, 0.8, 1.0);
  ...
  for(var i = 0; i < numObject; i++) 
  {
    if(i == selectNo) continue;//gl.uniform1i(cubemapLoc, true);
    else
    {
      gl.uniform1i(cubemapLoc, false);
...
function onClickFilmSpace()
{
  filmSpace = parseFloat(form2.filmSpace.value);
  display();
}
