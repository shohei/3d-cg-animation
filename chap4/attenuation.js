...
var light0;//光源オブジェクト（光源そのものをオブジェクトとして描画）
...
function init()
{
  ...
  form2.atten0.value = light.attenuation[0];
  form2.atten1.value = light.attenuation[1];
  form2.atten2.value = light.attenuation[2];
...
{  
  ...
  var attenLoc = gl.getUniformLocation(gl.program, 'u_attenuation');
  gl.uniform3fv(attenLoc, light.attenuation);
...
{
  light0.kind = "SPHERE";
  light0.ambient = [1.0, 1.0, 1.0, 1.0];
  light0.vSize = new Vector3(0.1, 0.1, 0.1);
  light0.vPos = new Vector3(light.pos[0], light.pos[1], light.pos[2]);
  light0.nSlice = 8;
  light0.nStack = 8;
...
function onClickAtten()
{
  light.attenuation[0] = parseFloat(form2.atten0.value);
  light.attenuation[1] = parseFloat(form2.atten1.value);
  light.attenuation[2] = parseFloat(form2.atten2.value);
  display();
}
