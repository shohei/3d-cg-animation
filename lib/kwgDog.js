...
Dog.prototype.initKeyAnimation = function()
{
  ...
  {
    this.k_vPos[i].v3_copy(this.vPos);//new Vector3();
    this.k_vEuler[i].v3_copy(this.vEuler);
    for(var j = 0; j < this.numJoints; j++) this.k_vJoints[i * this.numJoints + j].v3_copy(this.vJoints[j]);
  }
}
//----------------------------------------------------------------------------
//ﾌﾚｰﾑﾃﾞｰﾀ作成
 ...
 Dog.prototype.makeFrameData = function ()
{
  ...
  for(k = 1; k < this.numKeyFrame; k++)
  {
    ...
    vpos.v3_copy( v3_sub(this.k_vPos[k], this.k_vPos[k-1]) );
    var vang = new Vector3();
    vang.v3_copy( v3_sub(this.k_vEuler[k], this.k_vEuler[k-1]) );
  ...
  this.vPos.v3_copy(this.f_vPos[this.numFrame]);
  this.vEuler.v3_copy(this.f_vEuler[this.numFrame]);
  for(i = 0; i < this.numJoints; i++) this.vJoints[i].v3_copy(this.f_vJoints[this.numFrame*this.numJoints + i]);
}
//----------------------------------------------------------
//与えられた距離歩く
...
Dog.prototype.turn = function(angle, time)
{
  ...
  this.k_vEuler[1].v3_copy(this.k_vEuler[0]);
