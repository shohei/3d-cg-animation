...
function readyTexture() 
{
  ...
  image[0].src = '../imageJPEG/dog1.jpg';//particle.jpg';
  image[1].src = '../imageJPEG/matumae.jpg';
...
//  rigid[0].kind = "SPHERE";
  ...
  rigid[0].diffuse = [0.8, 0.8, 0.8, 1.0];
  rigid[0].ambient = [0.4, 0.4, 0.4, 1.0];
...
　//サンプラにユニット番号を設定
  var samplerLoc0 = gl.getUniformLocation(gl.program, 'u_sampler0');
  gl.uniform1i(samplerLoc0, 0);
  var samplerLoc1 = gl.getUniformLocation(gl.program, 'u_sampler1');
...
function onChangeMode()
{
  ...
  else if(radioM[1].checked) mode = 1;
  else mode = 2;
  display();
}
